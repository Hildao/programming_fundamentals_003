...
const catalogue = [
  ...
  { title: "The Yellow Wallpaper", author: "Charlotte Perkins Gilman", quantity: 1 },
  { title: "Conversations With Friends", author: "Sally Rooney", quantity: 10 },
...
function countBooksByAuthor(author) {
  ...
  for (let i = 0; i < catalogue.length; i++) {
    const booksTitle = catalogue[i].author;
    if (booksTitle === author) {
      count++;
    }
  }
  return count
...
function checkBookByTitle(title) {
  for (let i = 0; i < catalogue.length; i++) {
    const booksTitle = catalogue[i].title;
    if (booksTitle === title) {
      return true;
    }
  }
...
function countBooksByFirstLetter(letter) {
  ...
  for (i = 0; i < catalogue.length; i++) {
    ...
    let bookTitle = catalogue[i].title.charAt(0);
    //search for books begining with H
    if (bookTitle === letter) {
      count++
    }
...
function getBooksByAuthor(author) {
  let books = [];
  for (let i = 0; i < catalogue.length; i++) {
    const bookByAuthor = catalogue[i].title.author;
    if (bookByAuthor === author) {
      books.push(quantity);
    }
  }
}
...
function checkQuantity(title, quantity) {
  books = 0;
  for (i = 0; i < catalogue.length; i++) {
    const book = catalogue[i].title.quantity;
    book;
  }
}
